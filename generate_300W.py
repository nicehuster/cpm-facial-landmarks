...
from data import for_generate_box_str,load_list_from_folders
...
def return_box(image_path, pts_path, all_dict, USE_BOX):
  ...
  if USE_BOX == 'GTL':
    box_str = for_generate_box_str(pts_path, 68, 0)
  elif USE_BOX == 'GTB':
...
def generate_300w_list(root, save_dir, box_data, SUFFIX):
  ...
  with open(osp.join(save_dir, '300w.train.' + SUFFIX), 'w') as txtfile:
    for cpair in train_set:
      #box_str = for_generate_box_str(cpair[1], 68, EXPAND_RATIO)
...
if __name__ == '__main__':
  path_300w=osp.join('datasets','300W')
  USE_BOXES = ['GTB', 'DET']
  box_datas = load_all_300w(path_300w)
  this_dir = osp.dirname(os.path.abspath(__file__))
  SAVE_DIR = osp.join(this_dir, 'datasets','300W_lists')
